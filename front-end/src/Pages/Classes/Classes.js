...
export default function Classes() {
  ...
  const filterPostsBySubject = (educationLevel, subject) => {
    return posts.filter(
      (post) => post.edulevel === educationLevel && post.subject === subject
    );
  };
  ...
  const getUniqueSubjectsByEducationLevel = (educationLevel) => {
    const subjects = new Set(
      posts
        .filter((post) => post.edulevel === educationLevel)
        ...
        <div className="class-container">
          <div className="class-section1">
            ...
            <div className="class-images1">
              {getUniqueSubjectsByEducationLevel("A/L").map(
                (subject, index) => (
                  <div key={index}>
                    <h4>{subject}</h4>
                    
                      <ComponentClass
                        posts={filterPostsBySubject("A/L", subject)}
                      />
          ...
          <div className="class-section2">
            ...
            <div className="class-images2">
              {getUniqueSubjectsByEducationLevel("O/L").map(
                (subject, index) => (
                  <div key={index}>
                    <h4>{subject}</h4>
                    
                    <ComponentClass
                      posts={filterPostsBySubject("O/L", subject)}
                    />
          ...
          <div className="class-section3">
            ...
            <div className="class-images3">
              {getUniqueSubjectsByEducationLevel("Grade 5").map(
                (subject, index) => (
                  <div key={index}>
                    <h4>{subject}</h4>
                    
                    <ComponentClass
                      posts={filterPostsBySubject("Grade 5", subject)}
                    />
