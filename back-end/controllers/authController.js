...
import { sendForgotPasswordEmail , sendVerificationEmail } from "../config/sendEmails.js";
...
const registerController = async (req, res) => {
    ...
    try {
        ...
        if (sendMail) {
            return res.status(201).json({ success: true, msg: "Successfully registered. Error in sending verification email" });
        } else {
            return res.status(201).json({ success: true, msg: "Successfully registered" });
        }
    } catch (error) {
...
const forgotpasswordController = async (req, res) => {
    ...
    if (!emailRegex.test(email)) {
        ...
        if (sendMail) {
            return res.status(201).json({ success: true, msg: "Error in sending verification email" });
        } else {
...
const resetpasswordController = async (req, res) => {
    ...
    if (!newPassword || !confirmNewPassword || !email) {
        return res.status(401).json({ success: false, msg: "Please fill in all the fields" });
    }
